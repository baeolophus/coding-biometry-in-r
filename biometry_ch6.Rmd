...
R also has a quantile() function to make sample quantiles.  Type 5 uses the same equation as we did to make the variable "p".  The difference appears to be that quantile() tells you what values exist at given percentiles (the variables you put into the probs argument.)  In our case, we are calculating what percentile the sample is.  So we wouldn't use quantile() in this case despite its appealing name relative to step 3 below.
```{r}
...
The nesting of order orders those new numbers (the 4, 5, 2, 1,3) by the same procedure.  Order's result is always row indices.  Rank's result is always what rank the value is (1, 2, 3, etc), not its row index.  [Rank is equivalent to order(order).](http://stackoverflow.com/questions/38968911/why-does-orderorderx-equal-rankx-in-r?noredirect=1&lq=1)
