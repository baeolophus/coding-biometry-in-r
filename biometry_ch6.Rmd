...
First let's look at the ranking options for ties and the order() function
...
The rank order divided by n (sample size) will result in the first observation being small (1-1/2)/n but above 0 and the last observation being closer to 1.  These are [essentially percentiles](https://books.google.com/books?id=Xj3SBQAAQBAJ&pg=PA108&lpg=PA108&dq=p+%3D+(i+-1/2)/n+quantile&source=bl&ots=w5-cCqQMJE&sig=Ahu8gN1tFHPMCERhqghUv0qjnaI&hl=en&sa=X&ved=0ahUKEwijsoabpLPSAhUBHWMKHWrQAmsQ6AEIJTAC#v=onepage&q=p%20%3D%20(i%20-1%2F2)%2Fn%20quantile&f=false) if you multiplied them by 100.
Check out quantile().  Its type 5 is equivalent to this equation and is estimates of going backwards.  I.e., we're calculating the probs already (p) for the numbers we have.  
...
ordered_fakehouseflydata$quantilei <- quantile(x = ordered_fakehouseflydata$winglength,
         probs = ordered_fakehouseflydata$i/length(ordered_fakehouseflydata$winglength),
         type = 5)

ordered_fakehouseflydata$normal_quantilei <- qnorm(ordered_fakehouseflydata$quantilei,
                                                  mean = 0, 
                                                  sd = 1)
