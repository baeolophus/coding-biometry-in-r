...
```
The rank order divided by n (sample size) will result in the first observation being small (1-1/2)/n but above 0 and the last observation being closer to 1.
...
R does their opposite of the book so we have to use the datax=TRUE argument to get it to match.
```{r}
plot(y = ordered_fakehouseflydata$normal_quantile,
     x = ordered_fakehouseflydata$winglength,
     xlab = "Housefly wing lengths",
